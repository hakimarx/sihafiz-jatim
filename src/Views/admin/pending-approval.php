...
<?php else: ?>
    <form action="<?= APP_URL ?>/admin/pending/batch-approve" method="POST" id="batchForm">
        <?= csrfField() ?>
        <div class="card">
            <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                ...
                <button type="submit" class="btn btn-success btn-sm" id="btnBatchApprove" disabled>
                    <i class="bi bi-check-all me-1"></i>Setujui yang Dipilih
                </button>
            </div>
                    ...
                    <table class="table table-hover mb-0 align-middle">
                        ...
                        <tbody>
                            <?php foreach ($pendingUsers as $i => $pu): ?>
                                <tr>
                                    <td>
                                        <input class="form-check-input user-checkbox" type="checkbox" name="user_ids[]" value="<?= $pu['id'] ?>">
                                    </td>
                                    <td><?= $i + 1 ?></td>
                                    <td>
                                    ...
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <button type="button" class="btn btn-success" onclick="submitApprove(<?= $pu['id'] ?>)">
                                ...
                                <div class="modal fade" id="rejectModal<?= $pu['id'] ?>" tabindex="-1">
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-light" data-bs-dismiss="modal">Batal</button>
                                                <button type="button" class="btn btn-danger" onclick="submitReject(<?= $pu['id'] ?>)">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const btnBatch = document.getElementById('btnBatchApprove');
            ...
            function updateBatchButton() {
                ...
                if (checkedCount > 0) {
                    btnBatch.innerHTML = `<i class="bi bi-check-all me-1"></i>Setujui ${checkedCount} yang Dipilih`;
                } else {
                    btnBatch.innerHTML = `<i class="bi bi-check-all me-1"></i>Setujui yang Dipilih`;
                }
            }
        });
        ...
        function submitApprove(id) {
            ...
            form.method = 'POST';
            form.action = `<?= APP_URL ?>/admin/pending/${id}/approve`;
        ...
        function submitReject(id) {
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = `<?= APP_URL ?>/admin/pending/${id}/reject`;
            const csrf = document.createElement('input');
            csrf.type = 'hidden';
            csrf.name = 'csrf_token';
            csrf.value = '<?= generateCsrfToken() ?>';
