...
<head>
    ...
    <title><?= htmlspecialchars($title ?? 'Laporan Harian Hafiz') ?></title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="page">
        ...
        <div class="kop-surat">
            ...
            <div class="kop-logo">
                <!-- Second logo placeholder for symmetry -->
            </div>
        </div>

        <!-- Judul Laporan -->
        <div class="report-title">
            <h2>Laporan Harian Hafiz</h2>
        ...
        <div class="info-box">
            <table>
                <tr>
                    ...
                    <td>:</td>
                    <td><?= $bulanNames[(int)$bulan] ?? $bulan ?> <?= htmlspecialchars($tahun) ?></td>
                </tr>
                <tr>
        ...
        <?php if (!empty($laporan)): ?>
                ...
                <tbody>
                    ...
                    foreach ($laporan as $l): ?>
                        <tr>
                            ...
                            <td class="center"><?= date('d/m/Y', strtotime($l['tanggal'])) ?></td>
                            <td><?= htmlspecialchars($l['hafiz_nama'] ?? '-') ?></td>
                            <td><?= htmlspecialchars($l['desa_kelurahan'] ?? '-') ?></td>
                            <td><?= htmlspecialchars($l['jenis_kegiatan'] ?? $l['kegiatan'] ?? '-') ?></td>
            ...
            <p><strong>Total Laporan:</strong> <?= count($laporan) ?> data</p>
        <?php else: ?>
            <div style="text-align: center; padding: 40px; border: 1px solid #ccc;">
                <p><em>Tidak ada data laporan untuk periode ini.</em></p>
            </div>
        <?php endif; ?>
